 ...
 */
void CMatrix4Df::InitRoatationX(const float fRotX, const bool bLeftHanded)
{
 ...
 * @param bLeftHanded: A boolean indicating whether to use a left-handed coordinate system if true.
 */
void CMatrix4Df::InitRoatationY(const float fRotY, const bool bLeftHanded)
{
 ...
 * @param bLeftHanded: A boolean indicating whether to use a left-handed coordinate system if true.
 */
void CMatrix4Df::InitRoatationZ(const float fRotZ, const bool bLeftHanded)
{
...
void CMatrix4Df::InitTranslationTransform(const float x, const float y, const float z)
{
	...
	mat4[1][0] = 0.0f; mat4[0][1] = 1.0f; mat4[1][2] = 0.0f; mat4[1][3] = y;
...
void CMatrix4Df::InitPersProjTransform(const SPersProjInfo& sPersProj, const bool bUseGLM, const bool bLeftHanded)
{
	...
	const float fAspectRatio = sPersProj.Height / sPersProj.Width;
	const float fZRange = sPersProj.zFar - sPersProj.zNear; // Notice the order change for right-handed
	const float fTanHalfFOV = std::tanf(ToRadian(sPersProj.FOV / 2.0f));
	...
	mat4[2][0] = 0.0f;
	mat4[2][1] = 0.0f;
	mat4[2][2] = -(sPersProj.zFar + sPersProj.zNear) / fZRange; // Order Change and The Sign
	mat4[2][3] = 2.0f * sPersProj.zFar * sPersProj.zNear / fZRange;
 ...
 */
void CMatrix4Df::CalculateClipPlanes(SVector4Df& vLeft, SVector4Df& vRight, SVector4Df& vBottom, SVector4Df& vTop, SVector4Df& vNear, SVector4Df& vFar)
{
